...
function postAjax(url, data, success) {
    ...
    xhr.open('POST', url);
    xhr.onreadystatechange = function() {
        if (xhr.readyState>3 && xhr.status==200) { success(xhr.responseText); loadAcct();}
    };
    xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhr.send(params);
...
function getCheckVals(){
	...
	var allChecks = document.getElementsByClassName("noticeChecks");
	var cl = allChecks.length;
	
	var checkArray = [];
	for( var i = 0; i < cl; i++ ){
		checkArray.push(allChecks[i].id);
	}
	//alert(checkArray);
	var checkObj = {};
	checkArray.forEach(function(e, index){
