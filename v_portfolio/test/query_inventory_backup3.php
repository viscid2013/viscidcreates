...
try {
	...
	if(isset( $_REQUEST['search'] )){
	...
	else if(!isset( $_REQUEST['search'] ) && isset( $_REQUEST['filters'] ) ){
		...
		elseif( $fcount === 2 ){
			$filt0 = "%" . $filters[0] . "%";
			$filt1 = "%" . $filters[1] . "%";
		$stmt = $conn->prepare("SELECT iid, title, image_link, num_favs FROM inventory WHERE (formats LIKE :filter) XOR (formats LIKE :filter1)"); 
		$stmt->bindParam(':filter', $filt0);
		...
		elseif( $fcount === 3 ){
			...
			$filt1 = "%" . $filters[1] . "%";
			$filt2 = "%" . $filters[2] . "%";
		$stmt = $conn->prepare("SELECT iid, title, image_link, num_favs FROM inventory WHERE (formats LIKE :filter) XOR (formats LIKE :filter1) XOR (formats LIKE :filter2)"); 
		$stmt->bindParam(':filter', $filt0);
		$stmt->bindParam(':filter1', $filt1);
